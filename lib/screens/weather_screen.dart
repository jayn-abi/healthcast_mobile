                ...
                child: ConstrainedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // ✅ Animation: subtle shimmer sweep (keeps your widgets stateless)
                      const _TileSheen(
...
class _AnimatedWeatherCardState extends State<AnimatedWeatherCard>
  ...
  Animation<double> _bgShift = const AlwaysStoppedAnimation(0.0);
  Animation<double> _glow = const AlwaysStoppedAnimation(0.0);
  ...
  void initState() {
    ...
    _bgShift = CurvedAnimation(parent: _ambientCtrl, curve: Curves.easeInOut);
    _glow = CurvedAnimation(parent: _ambientCtrl, curve: Curves.easeInOut);
  }
                ...
                child: AnimatedBuilder(
                  ...
                  builder: (context, _) {
                    ...
                    final begin = Alignment(
                      -0.1 + (_bgShift.value * 0.2),
                      -1.0 + (_bgShift.value * 0.35),
                    );
                    final end = Alignment(
                      0.1 - (_bgShift.value * 0.2),
                      1.0 - (_bgShift.value * 0.25),
                    );

                    
                    final glowOpacity = 0.06 + (_glow.value * 0.06); 
                    return Container(
                      padding:
                          EdgeInsets.fromLTRB(16 * s, 14 * s, 16 * s, 14 * s),
                      ...
                      child: Column(
                        children: [
                          ...
                          TweenAnimationBuilder<double>(
                            key: const ValueKey('temp_countup'),
                            tween: Tween<double>(begin: 25, end: 28),
                          ...
                          Opacity(
                            opacity: 0.14 + (_bgShift.value * 0.08),
                            child: Divider(
                              color: TEXT_COLOR_WHITE,
                              height: 1,
                            ),
...
class _TileSheen extends StatefulWidget {
  final Widget child;
  ...
  @override
  State<_TileSheen> createState() => _TileSheenState();
}

class _TileSheenState extends State<_TileSheen>
  ...
  double _withPause(double v) {
    
    if (v <= 0.7) {
      return Curves.easeInOut.transform(v / 0.7);
    }
    return 1.0;
  }
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        final p = _withPause(_ctrl.value);
        final t = -1.3 + (p * 2.6); 
            ...
            Positioned.fill(
              child: IgnorePointer(
                child: ClipRRect(
                  ...
                  child: LayoutBuilder(
                    builder: (context, c) {
                      final sheenWidth = c.maxWidth * 0.6;

                      return Transform.translate(
                        offset: Offset(c.maxWidth * t, 0),
